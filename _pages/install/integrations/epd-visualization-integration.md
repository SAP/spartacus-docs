...
The SAP Enterprise Product Development Visualization Integration requires release **2105** of Commerce Cloud.

## Enabling the SAP Enterprise Product Development Visualization Integration in Spartacus

To enable the SAP Enterprise Product Development Visualization Integration, you need to configure:

- The Spartacus front end
- The SAP Commerce Cloud back end
- The SAP Enterprise Product Development Visualization back end

### Configuring the Spartacus Frontend
...
To use schematics to perform the full configuration execute the following:

```bash
ng add @spartacus/schematics --baseSite=powertools-epdvisualization-spa
```

When prompted to choose feature modules to include, you will need to ensure that the `EPD Visualization Integration` feature is included (along with any other features you require).
At a later point in the schematic execution, you will be prompted with `[EPD Visualization] What is the base URL (origin) of your EPD Fiori Launchpad? e.g. https://mytenant.epd.cfapps.eu20.hana.ondemand.com`.

Enter the [origin](https://developer.mozilla.org/en-US/docs/Glossary/Origin) portion of the URL that you use to access the EPD Fiori Launchpad for your tenant.
...
The root module of the SAP Enterprise Product Development Visualization Integration library provides the following default configuration, so the values below can be overridden, but otherwise do not need to be specified:
...
### Configuring the SAP Commerce Cloud Back End
...
The following steps describe how to configure the Commerce Cloud back end with sample data for the SAP Enterprise Product Development Visualization Integration.
...
2. Add the `epdvisualizationspartacussampledata` extension to the `localextensions.xml` file for your to your Commerce Cloud server.
3. Install the `epdvisualizationspartacussampledata` to your storefront as described in [Installing the `epdvisualizationspartacussampledata` Extension](installing-the-`epdvisualizationspartacussampledata`-extension).
4. Build and deploy, performing a database initialize. This will create a new database (destroying the existing one) containing the sample data for the SAP Enterprise Product Development Visualization Integration.

### `epdvisualizationspartacussampledata` Extension Initialization Actions

The `epdvisualizationspartacussampledata` extension initialization creates:
...
- `powertools-epdvisualization-spa` and `electronics-epdvisualization-spa` sites using the content catalogs above
- Product data in the `powertoolsProductCatalog` that corresponds to 3D and 2D data that can imported and prepared for use in visual spare part picking

### Installing the `epdvisualizationspartacussampledata` Extension

Perform the following to install the `epdvisualizationspartacussampledata` extension
...
For a CCv2 server this can be achieved by adding the following entry (along with any required comma separator characters) to the `storefrontAddons` array in the `core-customize/manifest.json` file:
...
The following help topic and its subtopics describe the configuration required in the SAP Enterprise Product Development tenant:

- [Configure Visualization Backend](https://help.sap.com/viewer/e56465f22e564c9690725351432cc906/Cloud/en-US/2a9de0ae0d4245c4b6599fe9cf66d51f.html)

The following help topic describes how to prepare visualizations in the SAP Enterprise Product Development system:

- [Prepare Visualizations for Storefront Part Picking](https://help.sap.com/viewer/e56465f22e564c9690725351432cc906/Cloud/en-US/d981e39ae1bb40a2b97cb93ff039b0be.html)

The following topic provides specific information relating to creating and preparing visualizations that correspond with the sample product data created by the `epdvisualizationspartacussampledata` extension:

- [Prepare Sample Visualizations](https://help.sap.com/viewer/e56465f22e564c9690725351432cc906/Cloud/en-US/ba46273cec114d0393ad3ad798a9be61.html)

## Accessing the Sample Data

When your backend and frontend have been appropriately configured, built and deployed, you can access 2D and 3D samples in the `powertools-epdvisualization-spa` site within your storefront.
