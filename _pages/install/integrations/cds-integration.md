...
- `baseUrl` asks you to provide the base URL for the Intelligent Selling Services back end (for example, `https://my-tenant-siteid.api.us.context.cloud.sap`)
...
@NgModule({
  ...
  providers: [
    provideConfig(<CdsConfig>{
      cds: {
        tenant: 'my-tenant',
        baseUrl: 'https://my-tenant-siteid.api.us.context.cloud.sap',
        endpoints: {
          strategyProducts:
...
- **baseUrl:** Replace the value shown in the example with the base URL of your Intelligent Selling Services environment. In particular, replace `my-tenant-siteid` with your actual Intelligent Selling Services tenant ID, and also replace `.us` with `.eu` if you are working with the EU environment of Intelligent Selling Services.
