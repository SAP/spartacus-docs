...
| `updatePaymentTransaction` | Updates an existing payment session context. Intended for subsequent updates after session creation with the provided payment session ID and configuration ID | `function updatePaymentTransaction(updatePaymentConfig:OpfPaymentUpdateConfig):Promise<OpfPaymentSessionData>` | `window.Opf.payments.global.updatePaymentTransaction({ paymentSessionld: 'session-456', config: { ... }}).then(sessionData => { console.log('Updated session:', sessionData); });`  |
