...
An unlocked group can be locked again by using the `ESC` key. The locked focus feature uses the escape focus feature, so that the host element is focused again when pressing `ESC`. For more information, see [Escape Focus]({{ site.baseurl }}{% link _pages/dev/accessibility/keyboard-focus/escape-focus.md %}).
