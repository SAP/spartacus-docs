...
### Lazy-loading TypeScript Translation Assets

The configuration `i18n.backend.loader` allows you to lazy load from a local project not only JSON files but also TypeScript/JavaScript files, if you want. The following is an example of how to load TypeScript translation assets, assuming your TypeScript files are located in the `assets/i18n-assets` folder:
...
providers: [
  provideConfig({
    i18n: {
      backend: {
        loader: (lng: string, ns: string) =>
          import(`../../assets/i18n-assets/${lng}/${ns}.ts`), // NOTE: .ts extension, but not .json
        chunks: translationChunksConfig,
      },
    },
  }),
...
Then you might also need to adjust your `tsconfig.json` file (e.g. `tsconfig.app.json`) and add the pattern `src/assets/i18n-assets/**/*.ts` to the `"include"` array, so that TypeScript files are included in the build process. The following is an example:
...
```json
{
  ...
  "include": [
    ...
    "src/assets/i18n-assets/**/*.ts"
  ]
}
```

**Note**: If you want to lazy load translations from TypeScript files, each file must export its translation object with the `export default` keywords. The following is an example:

```typescript
// some-translation-file.ts
export default {
  someKey: {
    someLabel: 'Value',
    someOtherLabel: 'Other Value',
  },
};
```
...
**Note:** Although it is possible to configure Spartacus to load translation content from external sources over HTTP, using insecure HTTP connections exposes your storefront to man-in-the-middle attacks by potentially allowing attackers to intercept and modify translation content. Accordingly, you **must not** use insecure HTTP connections when loading translation content. All external translation resources must be served over HTTPS to ensure integrity and to prevent tampering.
...
You can enhance the security of your Spartacus configuration with the following measures:

- Always explicitly enable escaping by setting `escapeValue: true` to prevent script injection.

- Avoid fetching or injecting translations from untrusted external sources unless they have been properly validated and sanitized.

- Avoid inserting raw HTML inside translations unless absolutely necessary. If you must allow HTML in translations, ensure proper sanitization before rendering content. For example, use Angular’s built-in sanitization mechanisms, such as DomSanitizer, to filter unsafe content.

- Avoid using the `-` prefix, such as `{{ '-myKey' | cxTranslate }}`, because it could introduce a potential security risk and lead to vulnerabilities in data handling and translations.

### Escaping Value

To prevent injection attacks through translations, ensure that escaping is enabled. You can do this by setting `escapeValue: true` in the interpolation settings, as shown in the following example:
