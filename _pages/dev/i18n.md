...
_Note: Since version 6.1, the i18n **config property `i18n.backend.loader`** has become available and is now **recommended for better SSR performance** than the old config property `i18n.backend.loadPath`. The latter should only be used when storing translations in a remote endpoint._

To enable lazy loading of JSON translation assets stored in a local project, configure the property `i18n.backend.loader` to be a function  returning a `Promise` with the translation chunk for the given language and chunk name. For example: 
...
We have configured a **function** that returns a **Promise (ES dynamic import in this case)** with the translation chunk. The ES dynamic import feature allows to split the JSON files from the main JS bundle and have them loaded by Webpack only when needed. During the build process, Webpack will wrap the JSON files into JS files. The following screenshot from the Chrome DevTools "Network" tab shows the example compiled JSON files loaded in the browser:
...
The screenshot below shows the same build artifacts as before, but in a production build. The filenames are obfuscated and hashed by Webpack, similarly to other production build artifacts.
...
**Note: When using SSR mode and dynamic import in `i18n.backend.loader`, the build artifacts are loaded directly from the local file system by Webpack, providing improved SSR performance compared to the previous configuration property `i18n.backend.loadPath` which required a separate HTTP request to load local JSON files.**

### Lazy-loading from a remote endpoint
If your translation assets are stored externally, to take advantage of lazy loading translation assets from a remote endpoint, you need to configure the property `i18n.backend.loadPath` to the URL to the JSON files using the {% raw %}`{{lng}}`{% endraw %} placeholder for language and the {% raw %}`{{ns}}`{% endraw %} placeholder for chunks, for example {% raw %}`https://example.com/{{lng}}/{{ns}}.json`{% endraw %}.
...
It is theoretically possible to use a local path (without `https://...`) for `loadPath`, e.g. {% raw %}`'assets/i18n-assets/{{lng}}/{{ns}}.json'`{% endraw %}. However, this is **not recommended** in Server Side Rendering (SSR) as it would cause translations to be loaded via http requests, resulting in unnecessary http roundtrips from SSR to the `ExpressJS` server itself. For better performance in SSR, it is advised to use the config `i18n.backend.loader` with dynamic import, described in the section above.
