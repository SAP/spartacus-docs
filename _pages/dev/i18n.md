...
You may configure Spartacus to load translation content from external sources over HTTP.  
Using insecure HTTP connections exposes this mechanism to man-in-the-middle (MITM) attacks, potentially allowing attackers to intercept and modify translation content.

#### Recommendation
You **must not** use insecure HTTP connections when loading translation content.  
All external translation resources **must be served over HTTPS** to ensure integrity and prevent tampering.
