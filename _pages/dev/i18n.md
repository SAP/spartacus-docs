...
**Note**: To maintain a smooth upgrade path, avoid adding custom keys to existing chunks in Spartacus. For more informatino, see the [Upgrading](#upgrading) section, below.
...
The following are the translation resources in the `cart.en.json` file for English:
...
For languages with more than two plural forms, numeric suffixes are used, such as `_0`, `_1`, ... `_5`, and so on. The following are the translation resources in the `cart.pl.json` file for Polish, which has three rules for pluralization:
...
The following special parameters interfere with the `i18next` translation mechanism, and Spartacus does not support them: `defaultValue`, `replace`, `lng`, `lngs`, `fallbackLng`, `ns`, `keySeparator`, `nsSeparator`, `returnObjects`, `joinArrays`, `postProcess`, `interpolation`, `skipInterpolation`
