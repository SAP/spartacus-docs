...
- Use Angular’s built-in sanitization mechanisms, such as DomSanitizer, to filter unsafe content when allowing HTML in translations.

- The '-' prefix, such as ```{{ '-myKey' | cxTranslate }}```, may introduce a potential security risk, as it can lead to vulnerabilities in data handling and translations.
