...
is done in the same module than where B2cStorefrontModule is also imported.  For most projects, this will be in the `AppModule`.  Because of the underlying mechanisms that the Spartacus configuration relies on, configuration overwrites have to be done this way.  Configuring translation overwrites in a sub module, for example, will not work, even if that module is imported after `B2cStorefrontModule`. 
