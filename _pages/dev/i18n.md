...
_Note: The i18n config property `loader` became available only since version 6.1. In earlier versions just the config property `loadPath` was available. Due to SSR performance reasons, it's recommended to use then new config property `loader` if translations are stored locally in the projects' assets. Please use the old config `loadPath` only for loading translations from remote endpoints._
 
To take advantage of lazy loading the local translation assets, you need to configure the `loader` function to return a `Promise` with the translation chunk for the given chunk name and language. The following is an example of how to do this:
...
We have configured a **function** that returns a **Promise** with the translation chunk. This is made possible by leveraging the **ES dynamic import** feature, which allows us to split the JSON files from the main JS bundle and have them loaded by Webpack only when needed. During the build process, Webpack will wrap the JSON files into JS files. The following example screenshot from the Chrome DevTools "Network" tab shows the compiled JSON files loaded in the browser:
...
This screenshot shows the same artifacts as before, but in a production build. The filenames have been obfuscated and hashed, similar to other production build artifacts of Webpack.
...
**Note: When using SSR mode, build artifacts are loaded directly from the local file system by Webpack, which is more efficient than loading them via an HTTP request in SSR.**

### Lazy-loading of translation assets from a remote endpoint
If your translation assets are stored externally, to take advantage of lazy loading translation assets from a remote endpoint, you need to configure the URL to the JSON files using the {% raw %}`{{lng}}`{% endraw %} placeholder for language and the {% raw %}`{{ns}}`{% endraw %} placeholder for chunks, for example {% raw %}`https://example.com/{{lng}}/{{ns}}.json`{% endraw %}.
...
It is theoretically possible to use a local path (without `https://...`) for `loadPath`, e.g. {% raw %}`'assets/i18n-assets/{{lng}}/{{ns}}.json'`{% endraw %}. However, this is **not recommended** in Server Side Rendering (SSR) as it would cause translations to be loaded via http requests, resulting in unnecessary http roundtrips from SSR to the assets served by the `ExpressJS` . For better performance in SSR, it is advised to use the **lazy-loading local assets approach** described in the section above.
...
**Note**: No feature lib (e.g. `@spartacus/cart`, `@spartacus/cdc`, ...) have the predefined JSON files with translations exported. If you want to use them, you need to create them manually. For tracking this issue, see https://github.com/SAP/spartacus/issues/9186.
