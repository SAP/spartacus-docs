...
Important: In the above example, the traslation overwrites configuration:
...
is imported in the same module than B2cStorefrontModule.  For most projects, this will be in the `AppModule`.  Because of the underlying mechanisms that the Spartacus configuration relies on, i18n translation overwrites have to be configured this way.  Configuring translation overwrites in a sub module, for example, will not work, even if that module is imported after `B2cStorefrontModule`.  The configuration mechanism as a whole may be improved in the future to remove this limitation.
