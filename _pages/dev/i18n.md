...
_Note: The i18n config property `i18n.backend.loader` became available only since version 6.1, and **is recommended now** due to SSR performance reasons over the old config property `i18n.backend.loader`. The old config property `i18n.backend.loadPath` should be used  only when storing translations in a remote endpoints._
