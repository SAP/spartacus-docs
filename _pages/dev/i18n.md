...
### Prefer cxTranslate pipe to TranslationsService in components
Generally in components we should prefer using the `cxTranslate` pipe in the html template pipe rather than the `TranslationService` in the TS logic (unless using `TranslationService` is justified). 

The disadvantages of using `TranslationService` are:
- it introduces an unnecessary constructor dependency to the component TS class 
- it _might_ introduce unnecessary methods and RxJs Observable logic to the component TS class
