...
Starting from the version 5.1, the Spartacus installer defines the `$styleVersion` value in the `<sourceRoot>/styles-config.scss` file. In this file, if you install Spartacus 5.1, you should find the following line:

```scss
$styleVersion: 5.1;
```

For Spartacus versions lower than 5.1, the installer defines the `$styleVersion` value in the `<sourceRoot>/styles.scss` file. In this file, if you install Spartacus 4.3, you should find the following line:
...
When you upgrade Spartacus, the `$styleVersion` that your app uses is not be updated. This way, backwards compatibility of styling is preserved. You can choose to manually opt in on the updated styling, but be aware that these updates may contain breaking changes. To opt in on the updated styling, you need to manually update the `$styleVersion` value to the feature level you wish to use.

For Spartacus 5.1 and beyond, to update the style version, update the `<sourceRoot>/styles-config.scss`.

Prior to Spartacus 5.1, to update the style version, update the `<sourceRoot>/styles.scss` and any other files where you might have `$styleVersion` overrides.


## Configuring Styling Versioning for Feature Libraries for Spartacus 5.1+

Starting from Spartacus 5.1, the styling configuration will propagate properly to the feature libraries.

Under the hood, the spartacus installer creates a styles config file `<sourceRoot>/styles-config.scss`. This file is used to define or override styling config variables.  Once it is generated by the installer, the `<sourceRoot>/styles-config.scss` file should contain the style version variable override:

```scss
$styleVersion: 5.1;
```

The `styles-config.scss` file is then imported in all the feature style root files to propagate the configuration in all the feature libraries as well. By default these files are:

- `src/styles.scss`
- `src/styles/spartacus/*.scss`

When you use Spartacus schematics to install a feature, if `<sourceRoot>/styles-config.scss` exists, it will be imported in the corresponding `src/styles/spartacus/*.scss` file.  If you upgraded from a prior version of Spartacus and your app does not have `<sourceRoot>/styles-config.scss`, then the import is simply skipped and the pre 5.1 behaviour prevails. 

## Using Styles Versioning After Upgrading and existing App to Spartacus 5.1

The upgrade schematics will not create the `<sourceRoot>/styles-config.scss` file. By default, your upgraded app will keep the pre 5.1 setup as far as the styles configuration goes. Optionally, you can manually create the `<sourceRoot>/styles-config.scss` file and import it in the feature root style files. This manual process is explained later in this guide.

## Configuring Styling Versioning for Feature Libraries for Spartacus < 5.1

If you use a Spartacus version prior to 5.1, the Spartacus installer initializes the `$styleVersion` value for the "core" or "main" styles file, `<sourceRoot>/styles.scss`. However, the installer does not propagate the `$styleVersion` override to the feature library styling files. Therefore, by default, the styling feature level of feature libraries will stick to the `.0` version of the major version you are using.
...
### Creating a Style Config File to Propagate Style Versioning (for Spartacus < 5.1)
