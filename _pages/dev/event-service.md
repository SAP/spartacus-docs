...
Not all RxJs Observables are good candidates for an event stream. A good example of this is the NgRx selectors. The Observable of the NgRx selector replays the latest updated value at the moment of the subscription, regardless of when the value was updated! But a valid event stream should emit a value only when the actual event happens.
