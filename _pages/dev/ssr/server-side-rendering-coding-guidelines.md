...
In SSR server there is one long-living NodeJs process which handles all http requests. On each request: it bootstraps the angular application, returns the HTML to the client and cleans up the app's resources. If any subscription created in any Angular (singleton) service is not disposed on app destroy, this subscription remains pending even after the app is destroyed, which causes a memory leak. In SSR Angular calls `ngOnDestroy` for services, so any subscriptions created by the service can be unsubscribed on service's the end of life.


But any pending subscriptions are not disposed automatically and need to be unsubscribed manually in ngOnDestroy method. It applies not only to components, but also to services. Especially in SSR any pending subscription created by any service 
