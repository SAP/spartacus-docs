...
- [Which validations exists out of the box ?](#existing-validations) 
...
In order to specify validations for certain field, JSON schema is enhanced by exposing property called "validations".  Property represents array of complex objects which are translated in dynamic forms validation functions and they consist of two properties:
...
```
- **maxValue** - Default Angular validator
- **minValue** - Default Angular validator
- **maxLength** - Default Angular validator
- **minLength** - Default Angular validator
- **number** - Triggers error message is input type is not number.
...
All arguments defined in JSON file represents inputs parameters for compareToCurrentDate function from DefaultFormValidators and they are added to the function in run-time. In our example based on mapping defined in configuration we will find following validation function:
...
```
and assign that function with input parameter 'shouldBeGreated' (operator in this case) to the corresponding field. In same way we can pass multiple input parameters via arguments attribute in case function requires more and add them to validation function respectively.

## How to override existing validation in custom App

In order to override some predefined validation, customers should be able to do that in their modules by referencing their custom functions with the same name as existing ones. Let's try to override compareToCurrentDate function, so that it just console logs customized message and returns always true for validation.  We will start in one of our modules by specifying following configuration:
```typescript
X-module.ts
....
      validators: {
...
....
```
From code snippet above we can see that for validation with name compareToCurrentDate we have new function defined in CategoryFormsModule. Functions has following implementation:
...
Procedure for adding new function is more or less the same as for override. First we need to define new function somewhere and its implementation can be similar to the one for override. After that we only have to expose new function via configuration and both of those things are describe in code block below:
```typescript
New validation function
static customNewFucntion(regex) {
    return (control: AbstractControl): ValidationErrors | null => {
...
....
 ...
 validators: {
        ....
        newValidation: {
          validator: CategoryFormsModule.customNewFucntion
        }
      }
.....
```
