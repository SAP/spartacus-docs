...
The URLs of product, category and brand pages can be [configured only in Spartacus]({% link pages/dev/routes/Route-Configuration.md %}). Content Pages have a configurable URL in the CMS, called *page label*.
...
**Note:** The optional `paramsMapping` config can be used for properties that have a different name than the route parameter (i.e. to map from `product.code` to `:productCode`). For more information, see [Configurable Router Links]({% link pages/dev/routes/Configurable-Router-Links.md %}).
